...
object Huffman {
  ...
  def times(chars: List[Char]) : List[(Char, Int)] = {
    ...
    timesRepeat(chars, chars).distinct
  }

  def timesRepeat(chars: List[Char], initialChars: List[Char]) : List[(Char, Int)] = {

    if (chars.isEmpty) {
      Nil
    }
    else {
      val pair: (Char, Int) = (chars.head, timesChar(chars.head, initialChars))
      pair :: timesRepeat(chars.tail, initialChars)
    }
  }

  def timesChar(char: Char, chars:List[Char]): Int = {

    if (chars.isEmpty) 0
    else if (char == chars.head)
      1 + timesChar(char, chars.tail)
    else
      timesChar(char, chars.tail)
  }
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
      ...
      if (freqs.isEmpty) Nil
        ...
        val list: List[(Char, Int)] = makeOrderedList(freqs)
        val pair = new Leaf(list.head._1, list.head._2)
        pair::makeOrderedLeafList(list.tail)
      }
    }

  def makeOrderedList(freqs: List[(Char, Int)]): List[(Char, Int)] = freqs match {
  ...
  def insertCodeTree(x: CodeTree, xs: List[CodeTree]): List[CodeTree] = xs match {
    ...
    case List() => List(x)
    case y :: ys => if (weight(x) <= weight(y)) x::xs else y :: insertCodeTree(x, ys)
  }
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = {
      ...
      if (trees.isEmpty) trees
        ...
        // add fork and keep the weight order
        insertCodeTree(fork, trees.tail.tail)
      }
    }
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      ...
      if (bits.isEmpty) List()
        ...
        def pair: (Char, List[Bit]) = decodeChar(tree, bits)
